...
export const changelog: ChangelogEntry[] = [
  {
    ...
    version: "v2.20.0",
    highlights: [
      "New Cash Allocation card on the Overview splits your capital three ways instead of one lumped 'in use' number: Committed (cash already out — stock lots and LEAPs), Reserved (cash-secured put collateral that only deploys if you get assigned), and Free (truly unencumbered). A segmented bar shows the mix at a glance, and the card now carries the current/base capital figure too. The same meter, in compact form, now sits in the Positions tab header (replacing the old single deployed bar). The Overview was de-cluttered to match: the old top KPI row (Current Capital / P&L / Open Trades) is gone — current capital lives on the allocation card, P&L and open positions are already shown in the performance card and positions table below.",
      "New Assignment Ladder — a forward look at every open cash-secured put grouped by expiration date, showing how much cash each date would deploy if assigned and your running free cash after each one. It lives right inside the Cash Allocation card; if you have a lot of expirations it shows the soonest five with a 'Show all' toggle. Each put is color-coded: red for in-the-money (likely to assign), gray for out-of-the-money, and a dashed outline when there's no live quote. A warning appears if full assignment would outrun your liquid cash and need margin or a roll. The compact meter on a portfolio's Positions tab links back to that portfolio's Overview for the full breakdown.",
      "Selling a covered call from a stock lot now pre-selects that lot in the Add Trade form — one less click. You can still change it, and adding a trade from elsewhere still asks you to pick the lot.",
      "Capital Concentration on the Overview now includes long calls and LEAPs, not just cash-secured puts and stock lots — so the percentages reflect everything your capital is in.",
    ],
  },
  {
    date: "2026-06-04",
  ...
  },
  {
    ...
    version: "v2.19.1",
    highlights: [
      "Admin trade edit now lets you move a trade to a different portfolio — useful when you added it to the wrong account. Trades linked to a stock lot stay put (unlink first).",
      "Expiration date picker now has clickable month and year dropdowns. Big help when adding LEAPS — no more clicking the next-month arrow 24 times.",
      "Long calls and puts now show their capital allocation on the Open Positions table — the premium you paid is the capital at risk, and it shows up as a row like CSPs do.",
      "Open Premium on the All Accounts overview no longer counts long calls or puts — that's premium you paid out, not collected. Only cash-secured puts and covered calls count, matching the per-portfolio number.",
